...
export function AppPage() {
  ...
  async function submit(event: FormEvent) {
    event.preventDefault(); const validationError = validateSummaryText(text); if (busy || validationError) { if (validationError && text.length > 0) setError(validationError); return } setBusy(true); setError('')
    try { const response = await request('/api/summaries', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text, language }) }); if (!response.ok) throw new Error(await summaryRequestError(response)); const data = await response.json() as Summary; const complete: SummaryDetail = { ...data, inputText: text }; detailCache.current.set(complete.id, complete); returnView.current = 'workspace'; setDetailTarget(data); setSelected(complete); setView('detail'); queueMicrotask(() => resultHeading.current?.focus()); void loadRecent() } catch (reason) { setError(reason instanceof Error ? reason.message : 'Özet oluşturulamadı.') } finally { setBusy(false) }
  }
  async function signOut() { await logout(); navigate('/login', { replace: true }) }

  const pinned = recent.filter(item => pinnedIds.includes(item.id)); const validationError = validateSummaryText(text); const invalid = Boolean(validationError); const inputDescription = text.length > 0 && validationError ? 'character-count input-validation retention-note' : 'character-count retention-note'
...
async function summaryRequestError(response: Response) { if (response.status === 429) return 'İstek sınırına ulaştınız. Lütfen bir dakika sonra yeniden deneyin.'; if (response.status === 413) return 'Gönderilen metin isteği çok büyük. Metni kısaltıp yeniden deneyin.'; if (response.status === 422) return 'Bu metinde özetlenebilecek yeterli ve anlamlı içerik bulunamadı. Lütfen daha açıklayıcı bir metin girin.'; if (response.status === 400) { try { const body = await response.json() as { errors?: unknown }; if (body.errors && typeof body.errors === 'object' && !Array.isArray(body.errors)) for (const key of ['text', 'language']) { const messages = (body.errors as Record<string, unknown>)[key]; if (Array.isArray(messages)) { const safe = messages.find((message): message is string => typeof message === 'string' && message.length <= 300); if (safe) return safe } } } catch { /* Safe fallback below. */ } return 'Metni ve özet dilini kontrol edip yeniden deneyin.' } return 'Özet şu anda oluşturulamadı. Lütfen yeniden deneyin.' }
