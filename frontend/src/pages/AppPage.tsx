...
function SidebarSection({ title, items, empty, onOpen }: { title: string; items: Summary[]; empty: string; onOpen: (item: Summary, trigger?: HTMLButtonElement) => void }) {
  const [filter, setFilter] = useState('')
  const filtered = items.filter(item => matchesSearch(item, filter))
  return <section className="sidebar-section"><h2>{title}</h2>{items.length > 3 && <div className="sidebar-search-box"><input type="search" placeholder="Özet ara…" aria-label={`${title} içinde ara`} value={filter} onChange={e => setFilter(e.target.value)} /></div>}{items.length === 0 ? <p>{empty}</p> : filtered.length === 0 ? <p>Aramaya uygun özet yok.</p> : <ul>{filtered.map(item => <li key={item.id}><button title={titleFor(item)} data-detail-trigger={`${item.id}:sidebar:${title}`} onClick={event => void onOpen(item, event.currentTarget)}>{titleFor(item)}</button></li>)}</ul>}</section>
}
function LibraryView({ items, pinnedIds, historyState, onOpen, onPin }: { items: Summary[]; pinnedIds: string[]; historyState: 'loading' | 'ready' | 'error'; onOpen: (item: Summary, trigger?: HTMLButtonElement) => void; onPin: (id: string) => void }) {
  const [searchQuery, setSearchQuery] = useState('')
  const filtered = items.filter(item => matchesSearch(item, searchQuery))
  return <section className="library-view" aria-labelledby="library-title"><div className="workspace-heading"><p className="section-kicker">Kitaplık</p><h1 id="library-title">Son özetleriniz</h1><p>En yeni 7 özetinizi burada görüntüleyebilirsiniz.</p></div>{items.length > 0 && <div className="library-search-box"><SearchIcon /><input type="search" placeholder="Özetlerde veya başlıklarda ara…" aria-label="Özetlerde ara" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} /></div>}{historyState === 'loading' && <p role="status">Özetler yükleniyor…</p>}{historyState === 'error' && <p className="error">Özetler şu anda yüklenemedi.</p>}{historyState === 'ready' && items.length === 0 && <div className="empty-state"><LibraryIcon /><h2>Kitaplığınız henüz boş</h2><p>İlk özetinizi oluşturduğunuzda burada görünecek.</p></div>}{historyState === 'ready' && items.length > 0 && filtered.length === 0 && <p className="empty-message">Aradığınız kriterlere uygun özet bulunamadı.</p>}<div className="summary-grid">{filtered.map(item => <SummaryCard key={item.id} item={item} pinned={pinnedIds.includes(item.id)} onOpen={onOpen} onPin={onPin} />)}</div></section>
}
...
function DetailView({ item, target, loading, error, feedbackPending, feedbackError, pdfPending, pdfError, pinned, onFeedback, onDownloadPdf, onPin, headingRef, onBack, onNew, onRetry }: { item: SummaryDetail | null; target: Summary | null; loading: boolean; error: string; feedbackPending: boolean; feedbackError: string; pdfPending: boolean; pdfError: string; pinned: boolean; onFeedback: (value: SummaryFeedback) => void; onDownloadPdf: (id: string) => void; onPin: (id: string) => void; headingRef: RefObject<HTMLHeadingElement | null>; onBack: () => void; onNew: () => void; onRetry: () => void }) {
  ...
  if (loading) return <section className="summary-detail detail-state" aria-live="polite"><button className="text-button" onClick={onBack}>← Geri dön</button><p role="status">Özet ayrıntıları yükleniyor…</p></section>;
  if (error || !item) return <section className="summary-detail detail-state"><button className="text-button" onClick={onBack}>← Geri dön</button><p className="error" role="alert">{error || 'Özet ayrıntıları kullanılamıyor.'}</p>{target && <button className="text-button" onClick={onRetry}>Yeniden dene</button>}</section>;
  ...
  function toggleSpeech() {
    ...
    if (speaking || window.speechSynthesis.speaking) {
      ...
      utterance.lang = item!.language === 'English' ? 'en-US' : 'tr-TR'
  ...
  const sourceLen = item.inputText.length
  const summaryLen = item.summary.length
  const sourceReadMins = Math.max(1, Math.ceil(sourceLen / 1000))
...
function VolumeIcon() { return <svg aria-hidden="true" viewBox="0 0 24 24"><path d="M11 5L6 9H2v6h4l5 4V5zM15.54 8.46a5 5 0 0 1 0 7.07M19.07 4.93a10 10 0 0 1 0 14.14" /></svg> }
function StopIcon() { return <svg aria-hidden="true" viewBox="0 0 24 24"><rect x="6" y="6" width="12" height="12" rx="2" /></svg> }
function CopyIcon() { return <svg aria-hidden="true" viewBox="0 0 24 24"><rect x="9" y="9" width="13" height="13" rx="2" /><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1" /></svg> }
