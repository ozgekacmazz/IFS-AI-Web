...
export function AdminOverviewPage() {
  ...
  useEffect(() => { const controller = new AbortController(); Promise.all([adminApi.statistics(request, controller.signal), adminApi.promptInfo(request, controller.signal)]).then(([nextStatistics, nextPrompt]) => { setStatistics(nextStatistics); setPrompt(nextPrompt); setState('ready') }).catch(error => { if ((error as Error).name !== 'AbortError') setState('error') }); return () => controller.abort() }, [request])
