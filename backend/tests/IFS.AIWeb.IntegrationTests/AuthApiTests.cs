...
public sealed class AuthApiTests(ApiFactory factory) : IClassFixture<ApiFactory>
{
    ...
    {
        var client = factory.Client(); var password = "a long safe passphrase"; var response = await Register(client, "CaseUser", password); Assert.Equal(HttpStatusCode.Created, response.StatusCode);
    ...
    {
        var username = "flow" + Guid.NewGuid().ToString("N")[..8]; var password = "a long safe passphrase"; var client = factory.Client(false); await Register(client, username, password);
    ...
    {
        var username = "admin" + Guid.NewGuid().ToString("N")[..8]; var password = "a long safe passphrase"; var client = factory.Client(false); await Register(client, username, password);
        ...
        Assert.Equal(HttpStatusCode.Forbidden, (await client.GetAsync("/api/auth/me")).StatusCode); Assert.Equal(HttpStatusCode.Unauthorized, (await client.SendAsync(Request(HttpMethod.Post, "/api/auth/refresh", cookie))).StatusCode); Assert.Equal(HttpStatusCode.Unauthorized, (await client.PostAsJsonAsync("/api/auth/login", new { username, password })).StatusCode);
    }
    [Fact] public async Task ConcurrentRefresh_AllowsOnlyOneRotationAndRevokesTheFamily()
    {
        var username = "race" + Guid.NewGuid().ToString("N")[..8]; var password = "a long safe passphrase"; var client = factory.Client(false); await Register(client, username, password);
