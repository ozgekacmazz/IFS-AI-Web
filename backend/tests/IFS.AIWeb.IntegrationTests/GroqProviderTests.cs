...
public sealed class GroqProviderTests
{
    ...
    { var handler = new CaptureHandler(HttpStatusCode.OK, "{\"choices\":[{\"message\":{\"content\":\"{\\\"quality\\\":\\\"sufficient\\\",\\\"summary\\\":\\\"safe summary\\\"}\"}}]}"); var provider = Provider(handler); const string source = "A sufficiently descriptive source for provider contract testing."; var length = new SummaryLengthPolicy().Select(source); var prompt = new SummarizationPromptBuilder().Build(source, IFS.AIWeb.Domain.SummaryLanguage.English, length); var result = await provider.SummarizeAsync(prompt, default); Assert.Equal("safe summary", result.Text); Assert.Equal(SummaryContentQuality.Sufficient, result.Quality); Assert.Equal("Bearer", handler.Request!.Headers.Authorization!.Scheme); Assert.Equal("https://api.groq.com/openai/v1/chat/completions", handler.Request.RequestUri!.ToString()); var body = handler.Body!; Assert.Contains("openai/gpt-oss-120b", body); Assert.Contains("max_completion_tokens\":500", body); Assert.Contains("temperature\":0", body); Assert.Contains("reasoning_effort\":\"low\"", body); Assert.Contains("response_format", body); Assert.Contains("json_schema", body); Assert.Contains("strict\":true", body); Assert.Contains("additionalProperties\":false", body); Assert.Contains("required\":[\"quality\",\"summary\"]", body); Assert.DoesNotContain("maxLength", body); Assert.Contains($"no more than {length.EffectiveMaximumRunes} Unicode characters", body); Assert.Contains("quality", body); Assert.Contains("sufficient", body); Assert.Contains("insufficient", body); Assert.Contains("system", body); Assert.Contains("source_text", body); Assert.DoesNotContain("test-key", body); }
    ...
    [Fact] public async Task ProviderLogsSafeRequestResponseAndFailureMetadataWithoutSensitiveData()
    { const string sentinel = "RAW-PRIVATE-RESPONSE"; var logger = new CaptureLogger(); await Assert.ThrowsAsync<LlmProviderException>(() => Provider(new CaptureHandler(HttpStatusCode.BadRequest, $"{{\"error\":{{\"message\":\"{sentinel}\",\"type\":\"invalid_request_error\"}}}}"), logger).SummarizeAsync(new("summary-v6", "system", "SOURCE-PRIVATE", IFS.AIWeb.Domain.SummaryLanguage.English), default)); var log = string.Join(Environment.NewLine, logger.Messages); Assert.Contains("English", log); Assert.Contains("openai/gpt-oss-120b", log); Assert.Contains("500", log); Assert.Contains("json_schema", log); Assert.Contains("InvalidRequest", log); Assert.Contains("summary-v6", log); Assert.DoesNotContain(sentinel, log); Assert.DoesNotContain("SOURCE-PRIVATE", log); Assert.DoesNotContain("test-key", log); Assert.DoesNotContain("Bearer", log); }
