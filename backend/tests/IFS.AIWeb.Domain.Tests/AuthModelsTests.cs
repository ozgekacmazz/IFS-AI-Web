...
public sealed class AuthModelsTests
{
    ...
    { var now = DateTimeOffset.UtcNow; var replacement = Guid.NewGuid(); var token = RefreshToken.Create(Guid.NewGuid(), Guid.NewGuid(), "hash", Guid.NewGuid(), now, now.AddDays(7), now.AddDays(30)); token.Revoke(now.AddMinutes(1), "rotation", replacement); token.Revoke(now.AddMinutes(2), "other"); Assert.Equal(replacement, token.ReplacedByTokenId); Assert.Equal("rotation", token.RevocationReason); }
