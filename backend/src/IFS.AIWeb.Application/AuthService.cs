    ...
    IPasswordService passwords, IAccessTokenService accessTokens, IRefreshTokenService refreshTokens, IClock clock)
{
    ...
    {
        var (display, normalized) = AuthValidation.Username(command.Username);
        var first = AuthValidation.Name(command.FirstName, "firstName"); var last = AuthValidation.Name(command.LastName, "lastName");
