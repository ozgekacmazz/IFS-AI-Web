...
|---|---|---|---|---|---|---|
| PDF-M-01 | Tanım, s. 1 | Kullanıcının yazdığı paragraf/metin, arka plandaki LLM tarafından özetlenerek döndürülmelidir. | Kimliği doğrulanmış kullanıcıdan metni alan özetleme kullanım senaryosu ve değiştirilebilir LLM bağdaştırıcısı. | React ana sayfa; Application; Infrastructure LLM; API | Geçerli metin gönderildiğinde LLM isteği ve ekranda özet sonucu entegrasyon testiyle doğrulanır. | Phase 3'te uygulandı |
| PDF-M-02 | Tanım, s. 1 | Temel akış prompt tabanlı olmalıdır: kullanıcı metni girer, sistem LLM'e iletir, model özeti döndürür. | Kullanıcı girdisini kontrollü sistem talimatından ayıran, sürümlenmiş prompt oluşturucu. | Application; Infrastructure LLM | Sahte sağlayıcıyla oluşturulan isteğin metni içerdiği ve yanıtın geri aktarıldığı test edilir. | Phase 3'te uygulandı |
...
| PDF-M-04 | Kapsam ve Kullanıcı Akışı, ss. 1-2 | User ana sayfasında tek metin alanı, “Özetle” düğmesi ve sonuç alanı bulunmalıdır. | Erişilebilir form; gönderim, bekleme, başarı ve hata durumları. | React ana sayfa | Tarayıcıda metin girme, gönderme ve sonuç gösterme akışı doğrulanır. | Phase 3'te uygulandı |
| PDF-M-05 | Kapsam, s. 1 | Admin panelini yalnızca admin rolü görmelidir. | Menü görünürlüğü ve sunucu tarafı rol politikası birlikte uygulanır. | React yönlendirme; API yetkilendirme (authorization) | User için menünün gizli ve admin URL/API çağrılarının 403 olduğu; admin erişiminin başarılı olduğu test edilir. | Uygulandı |
| PDF-M-06 | Kapsam; Admin Akışı, ss. 1-2 | Admin log ekranında kullanıcı metinleri ve/veya AI özet yanıtları listelenmelidir. | Tarih, kullanıcı, kısa giriş ve kısa özet içeren sayfalı kayıt listesi; tam içerik Admin'e açılmaz. | React admin log; Application; Infrastructure persistence; API | Yetkili listeleme, alan eşleşmesi, kısaltma ve yetkisiz erişim test edilir. | Uygulandı |
| PDF-M-07 | Admin Akışı, s. 2 | Her istek/yanıt en basit hâliyle tarih-kullanıcı-kısa giriş-kısa özet içeren tek satırlık kayıt olarak listelenmelidir. | Her özetleme denemesi için güvenli kompakt kayıt; başarısız/süresi dolmuş içerik gösterilmez. | Application; PostgreSQL; admin UI | Satırın doğru kullanıcı, tarih ve güvenli önizlemelerle oluştuğu doğrulanır. | Uygulandı |
| PDF-M-08 | Kapsam; Admin Akışı, ss. 1-2 | Admin yeni kullanıcı oluşturabilmelidir; kullanıcı adı/e-posta, şifre ve user/admin rolü atanabilmelidir. | Admin kullanıcı oluşturma formu ve yetkili API kullanım senaryosu. | React admin kullanıcılar; Application Identity; API; PostgreSQL | Admin başarı/çakışma/geçersiz rol; user için 403 senaryoları test edilir. | Uygulandı; kimlik olarak kullanıcı adı kullanılır |
| PDF-M-09 | Kapsam; Admin Akışı, ss. 1-2 | Admin kullanıcı şifresini güncelleyebilmeli veya sıfırlayabilmelidir. | Refresh oturumlarını geçersizleştiren yönetici şifre sıfırlama akışı. | React admin kullanıcılar; Application Identity; API | Yeni şifreyle giriş, eski şifrenin reddi ve yetki kontrolü doğrulanır. | Uygulandı |
| PDF-M-10 | Kapsam; Admin Akışı, ss. 1-2 | Admin kullanıcı durumunu Aktif/Pasif olarak güncelleyebilmelidir. | Eşzamanlı son-admin ve kendi hesabını pasife alma korumalı durum değiştirme. | React admin kullanıcılar; Application Identity; API; PostgreSQL | Pasife alma, giriş/erişim reddi ve tekrar aktifleştirme test edilir. | Uygulandı |
...
| PDF-M-12 | Yetkilendirme, s. 2 | Admin, Admin Paneli menüsünü görmeli; log ve kullanıcı yönetimi sayfalarına erişmelidir. | Admin navigasyonu ve rol politikaları. | React admin; API yetkilendirme | Admin rolüyle menü, sayfalar ve API çağrıları doğrulanır. | Uygulandı |
| PDF-M-13 | AI Entegrasyonu, s. 3 | Sistem bir LLM ile entegre edilmelidir; kullanılacak LLM sağlayıcısı yazılımcıya bırakılmıştır. | Sağlayıcıdan bağımsız arayüz; ilk sağlayıcı Groq ve değiştirilebilir model `openai/gpt-oss-120b`. | Application port; Infrastructure provider | Sahte HTTP işleyicili sağlayıcı sözleşme ve API entegrasyon testleri. | Phase 3'te uygulandı |
...
| PDF-M-15 | Güvenlik, s. 3 | Admin paneline doğrudan URL ile rol kontrolü aşılmadan erişilememelidir. | Her admin API uç noktasında sunucu tarafı admin politikası; istemci kontrolü yalnızca UX içindir. | API yetkilendirme; React router | User/anonim doğrudan URL ve doğrudan API çağrılarının reddedildiği test edilir. | Uygulandı |
...
| ID | Kaynak | Gereksinim | Planlanan uygulama | Katman / bileşen | Doğrulama | Durum |
|---|---|---|---|---|---|---|
| PDF-O-01 | Kullanıcı Akışı, s. 2 | Kullanıcı son üç özetini basit listede görebilir (**opsiyonel**). | Ürün kararıyla Phase 3 kapsamına alındı ve en yeni yedi uygun başarılı kayda genişletildi; yalnız mevcut kullanıcının kayıtları gösterilir. | React ana sayfa; Application; PostgreSQL | Kullanıcı yalnızca kendi en yeni yedi uygun başarılı kaydını doğru sırada görür. | Phase 3'te uygulandı |
...
| ID | Kaynak | Gereksinim | Planlanan uygulama | Katman / bileşen | Doğrulama | Durum |
|---|---|---|---|---|---|---|
| PDF-B-01 | Bonus, s. 4 | Kullanıcı başına dakika/işlem oran sınırlaması. | Kimliği doğrulanmış kullanıcı kimliğiyle, API örneği başına dakikada beş özetleme isteği. | API rate limiting | Beş istek başarı, altıncı istek 429 ve kullanıcı bölümlerinin ayrılığı test edilir. | Phase 3'te uygulandı |
| PDF-B-02 | Bonus, s. 4 | Çok uzun metinlerde loga yalnızca ilk X karakteri yazan girdi maskeleme. | En fazla 160 Unicode metin öğeli, daima eksiltilmiş önizleme; başarısız/süresi dolmuş içerik yok. | Application; admin API | Tam girdinin kompakt JSON veya uygulama loglarına düşmediği doğrulanır. | Uygulandı |
| PDF-B-03 | Bonus, s. 4 | Admin panelinde son yedi gün özet sayısını gösteren mini grafik. | Tam yedi UTC gününü sıfır kovalarla döndüren toplulaştırma uç noktası. | React admin; Application query; API | Tarih sınırları, saat dilimi ve grafik değerleri test edilir. | Uygulandı |
| PDF-B-04 | Bonus, s. 4 | Özet dilini (ör. Türkçe/İngilizce) seçebilme. | Ürün kararı gereği Türkçe/İngilizce seçimi MVP kapsamına alınır. | React ana sayfa; prompt builder | Her dil seçiminin kontrollü prompta aktarıldığı doğrulanır. | Phase 3'te uygulandı |
...
| ACIK-02 | Kayıt yetkisi | **Çözüldü:** Açık kayıt yalnızca `User` oluşturur; Admin paneli yetkili olarak `User` veya `Admin` oluşturabilir. | İlk Admin kontrollü seed mekanizmasıyla oluşturulur. |
| ACIK-03 | Log içeriği | **Kısmen çözüldü:** Tam girdi ve başarılı tam özet 30 gün saklanır; başarısız denemede güvenli üst veri tutulur. Admin ekranı yalnız daima eksiltilmiş kısa önizlemeleri gösterir. | Zamanlanmış fiziksel temizleme sonraki fazdadır. |
...
| ACIK-06 | LLM seçimi | **Phase 3'te kısmen çözüldü:** İlk sağlayıcı Groq, değiştirilebilir model `openai/gpt-oss-120b`, timeout 30 saniye ve çıktı sınırı 500 tokendır. | Veri işleme bölgesi, üretim kotası ve maliyet canlıya çıkıştan önce doğrulanacak; mimari sağlayıcıdan bağımsız kalır. |
